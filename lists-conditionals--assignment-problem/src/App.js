  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <input type="text" onChange={(event) => this.inputFieldChanged(event)} />
        ...
        {this.state.input.split('').map((letter, index) => {
          return (
            <CharComponent char={letter} key={index} click={() => this.charDelete(index)} />
          )
        })}
